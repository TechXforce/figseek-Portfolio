        ...
        <div className="relative w-20 h-20 md:w-60 md:h-60">
          ...
          <div className="relative w-full h-full rounded-full p-[2px] md:p-1 bg-gradient-to-br from-blue-500 to-purple-500">
            <div className="relative w-full h-full rounded-full overflow-hidden bg-black">
              <ImageWithFallback src="/images/hero.jpg" alt="Profile" className="w-full h-full object-cover" />
            </div>
          </div>
        </div>
      </motion.div>

      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center pt-16 md:pt-0 pb-16 md:pb-0">
        <h1 className="text-5xl md:text-9xl font-bold mb-6 min-h-[60px] md:min-h-[160px] mt-20 md:mt-8">
          <span className="bg-gradient-to-r from-white via-blue-400 to-purple-500 bg-clip-text text-transparent ml-0 md:ml-4">
            {displayedText.name}
          </span>
        </h1>
        <div className="mb-6 min-h-[80px] md:min-h-[160px]">
          <p className="text-lg md:text-3xl text-gray-300 mb-4">
            {displayedText.title}
        ...
        <motion.div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-8 md:mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7 }}>
          <Button size="sm" className="md:size-lg bg-transparent border-2 border-purple-500 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 hover:text-black hover:bg-gradient-to-r hover:from-blue-500 hover:to-purple-500 hover:bg-clip-border hover:border-transparent transition-all duration-300 text-sm md:text-base" onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}>
            Get In Touch
          </Button>
        ...
        <motion.div className="flex justify-center gap-4 md:gap-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.9 }}>
          <a href="https://github.com/TechXforce" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors"><Github className="w-5 h-5 md:w-6 md:h-6" /></a>
          <a href="https://zw.linkedin.com/in/kudakwashe-jabulani-chigiya-0a360b247" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors"><Linkedin className="w-5 h-5 md:w-6 md:h-6" /></a>
          <a href="https://wa.me/263713975875" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-green-400 transition-colors"><WhatsAppIcon className="w-5 h-5 md:w-6 md:h-6" /></a>
          <a href="mailto:www.kjchigiya@gmail.com" className="text-gray-400 hover:text-white transition-colors"><Mail className="w-5 h-5 md:w-6 md:h-6" /></a>
        </motion.div>
      </div>
      <motion.div className="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2" animate={{ y: [0, 10, 0] }} transition={{ duration: 2, repeat: Infinity }}>
